...
{
  /* USER CODE BEGIN 1 */
	SARA_R5_operator_stats opRet[MAX_OPS];
	uint8_t opsSeen;
  /* USER CODE END 1 */
...
// Get operators information, "May take up to 3 MINUTES !! ."
  opsSeen = saraR5GetOperators(opRet, MAX_OPS, resposta, sizeof(resposta));

  if (opsSeen > 0) {

	  printf("Connected to the network. Operators available: %d\n", opsSeen);
	  // Print details of the operators
	  for (int i = 0; i < opsSeen; i++) {
		  printf("Operator %d: %s, %s, %lu\n", i, opRet[i].longOp, opRet[i].shortOp, opRet[i].numOp);
	  }
	  // Clear buffer before reusing it
	  memset(resposta, 0, STANDARD_RESPONSE_BUFFER_SIZE);

// Proceed with PDP actions
	  ...
	  if (saraR5PerformPDPaction(1, SARA_R5_PSD_ACTION_ACTIVATE, resposta, sizeof(resposta)) != SARA_R5_ERROR_SUCCESS) {
		  printf("performPDPaction (activate profile) failed!\n");
	  }

  }else{
	  printf("No operators detected. Check network connection. Freezing...\n");
	  while(1);
  }
